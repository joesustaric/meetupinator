...
module MeetupThingy
  class App < Thor
    ...
    desc "getevents", "Write all upcoming events for the given meetup groups specified in INPUT to OUTPUT"
    ...
    def get_events()
      ...
      events = @event_finder.get_events_for_meetups group_names, @api
    ...
    def init
      @event_finder= @event_finder || MeetupThingy::EventFinder.new
      @event_list_file_writer= @event_list_file_writer || MeetupThingy::EventListFileWriter.new
      @api= @api || MeetupThingy::MeetupAPI.new(options[:meetup_api_key])
    end
